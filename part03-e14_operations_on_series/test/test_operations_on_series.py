...
#        with patch(patch_name(module_name, "pd.core.series.Series"), wraps=pd.core.series.Series) as ps:
        with patch(ph("pd.Series"), wraps=pd.Series) as ps:
            ...
            self.assertEqual(len(
                ret), 2, msg="Expected a pair of Series as a return value from function create_series!")
            s1, s2 = ret
            # ps.assert_called()
            self.assertEqual(
                ps.call_count, 2, msg="Expected the constructor pd.Series to be called exactly twice!")
    ...
    def test_modification(self):
        ...
        self.assertEqual(
            len(ret), 2, msg="Expected modify_series to return a pair of Series!")
        t1, t2 = ret
        self.assertIsInstance(
            t1, pd.Series, msg="Expected modify_series to return a pair of Series!")
        self.assertIsInstance(
            t2, pd.Series, msg="Expected modify_series to return a pair of Series!")
        t1_ind = list("abcd")
        t2_ind = list("ac")
